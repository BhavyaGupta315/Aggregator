 ...
 * for the full rationale. Certificates are exchanged in AUTH_CHUNK_SIZE pieces
 * (never as one big APDU): some NFC controllers — e.g. the Galaxy Tab Active5 —
 * cannot receive extended-length APDUs in HCE mode, so a ~1.3 KB single-APDU
 * cert arrives truncated and gets rejected as malformed. The reader uploads its
 * cert in chunks via AUTH_CRT and then pulls the card's cert via AUTH_CRG.
 */
object NfcAuth {
