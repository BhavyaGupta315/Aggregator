...
class WifiDirectTransferActivity : AppCompatActivity() {
    ...
    private fun beginWifiDirect() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            manager.requestDeviceInfo(channel) { device -> device?.let { showQr(it) } }
        }
        discoverPeers()
        statusText.text = if (direction == DIRECTION_SEND) "Show this QR to the receiver" else "Scan the sender QR"
        logStep(if (direction == DIRECTION_SEND) "Waiting for Wi-Fi Direct receiver..." else "Ready to scan.")
    }
    ...
    private fun showQr(device: WifiP2pDevice) {
        if (direction != DIRECTION_SEND || device.deviceName.isBlank()) return
    ...
    private suspend fun startServer() = withContext(Dispatchers.IO) {
        try {
            ...
            withContext(Dispatchers.Main) { logStep(e.message ?: "Socket closed") }
        }
    }
    ...
    private suspend fun runReaderProtocol(link: SocketTransceiver) {
        ...
        val peerPublicKey = if (CryptoUtils.CERT_AUTH_ENABLED) WifiAuth.exchangeCerts(link) else CryptoUtils.getOtherPublicKey()
        ...
        response = WifiAuth.sendChunked(link, CryptoUtils.CMD_AUTH_SEND_SIG, CryptoUtils.rsaSign(encryptedKey, myPrivateKey))
        ...
        logStep("Secure connection established.")
...
private object WifiAuth {
    ...
    fun sendChunked(link: SocketTransceiver, cmd: ByteArray, payload: ByteArray): ByteArray {
        ...
        while (true) {
            ...
            val frame = Utils.concatArrays(cmd, byteArrayOf(if (last) CryptoUtils.AUTH_CHUNK_LAST else CryptoUtils.AUTH_CHUNK_MORE), payload.copyOfRange(offset, end))
    ...
    private fun fetchPeerCert(link: SocketTransceiver): String {
        ...
        repeat(64) {
            ...
            if (res[0] == CryptoUtils.AUTH_CHUNK_LAST) return String(out.toByteArray(), Charsets.UTF_8)
        }
...
private class WifiApduProcessor(
    ...
    fun process(command: ByteArray): ByteArray {
        ...
        if (CryptoUtils.CERT_AUTH_ENABLED && command.size > CryptoUtils.CMD_AUTH_SEND_CERT.size &&
            ...
            peerPublicKey = try { CryptoUtils.verifyPeerCert(String(certBytes, Charsets.UTF_8)) }
            catch (e: PeerCertException) {
        ...
        val raw = when (snapshot.mode) {
            ...
            "FILE" -> handleSingleFile(command)
            "MULTI_FILE" -> handleMultiFile(command)
            else -> Utils.FILE_NOT_READY_SW
        }
        return if (raw.size > 2 && raw.isSuccess()) Utils.concatArrays(CryptoUtils.xorEncryptDecrypt(raw.getData(), sessionKey!!), Utils.SELECT_OK_SW) else raw
    }
    ...
    private fun handleSingleFile(command: ByteArray): ByteArray =
    ...
    private fun handleBytes(command: ByteArray, content: ByteArray?, label: String, mode: String): ByteArray {
        ...
        return when {
            ...
            Arrays.equals(command, Utils.GET_NEXT_DATA_CHUNK_COMMAND) -> {
                ...
                if (remaining <= 0) return Utils.SELECT_OK_SW
